...
{
    public function export(Tour $tour, Request $request)
    {
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();

        [$startRow_1, $maxLetter_1, $maxRow_1] = $this->genFirstTable($sheet, $tour);

        [$startRow_2, $maxLetter_2, $maxRow_2] = $this->genSecondTable($maxRow_1 + 2, $sheet, $tour);

        // get column index by letter
        $columnIndex = Coordinate::columnIndexFromString($maxLetter_2);
        [$startCell, $startRow_3, $maxLetter_3, $maxRow_3] = $this->getThirdTable(
            $columnIndex - 1,
            $maxRow_2 + 2,
            $sheet,
            $tour
    ...
    public function genFirstTable(Worksheet $sheet, Tour $tour): array
    {
    ...
    public function getThirdTable(string $startLetter, int $startRow, Worksheet $sheet, Tour $tour): array
    {
        $operator = "-";
        $profit = $tour->price - $tour->expenses;
        $operatorProfit = 0;
    ...
    public function genSecondTable(int $startRow, Worksheet $sheet, Tour $tour): array
    {
